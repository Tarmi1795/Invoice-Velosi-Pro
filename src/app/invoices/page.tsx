  ...
  return (
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4 grid grid-cols-2 gap-4">
          {formError && (
              ...
              <div className="space-y-2 col-span-2">
                ...
                >
                  ...
                  {inspections.map((opt: any) => (
                    <option key={opt.id} value={opt.id}>{opt.report_no || opt.id}</option>
                  ))}
                </select>
              </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300">CREDIT MEMO NO</label>
              <input 
                type="text" 
                step="any"
                className="input" 
                value={formData.credit_memo_no} 
                onChange={ev => setFormData({...formData, credit_memo_no: ev.target.value})} 
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300">CREDIT MEMO AMOUNT</label>
              <input 
                type="number" 
                step="any"
                className="input" 
                value={formData.credit_memo_amount} 
                onChange={ev => setFormData({...formData, credit_memo_amount: ev.target.value})} 
              />
            </div>
      ...
      <EnhancedBatchUploadModal 
          ...
          onSuccess={fetchData}
          expectedHeaders={['inspection_id', 'itp_po_id', 'po_no', 'sr_so_no', 'proforma_inv_no', 'proforma_inv_date', 'sap_sales_order', 'invoice_no', 'invoice_date', 'conso_invoice_no', 'conso_filename', 'total_amount', 'credit_memo_no', 'credit_memo_amount', 'payment_status']}
      />
    </div>
  );
}
