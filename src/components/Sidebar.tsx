...
export function Sidebar() {
  ...
  const [invoicesOpen, setInvoicesOpen] = useState(pathname.startsWith("/invoices"));

  const invoiceNavItems = [
    { name: "Proforma Invoices", href: "/invoices/proforma", icon: FileText },
    { name: "Final Invoices", href: "/invoices/final", icon: FileText },
  ];
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-3">
              <FileText size={20} className={pathname.startsWith("/invoices") ? "text-orange-500" : "text-gray-400"} />
              <span>Invoices</span>
            </div>
            {invoicesOpen ? <ChevronDown size={16} className="text-gray-400" /> : <ChevronRight size={16} className="text-gray-400" />}
          </button>
